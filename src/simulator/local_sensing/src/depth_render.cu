...
DepthRender::DepthRender() : cloud_size(0), host_cloud_ptr(NULL), dev_cloud_ptr(NULL), has_devptr(false)
{
}
...
void DepthRender::render_pose(double* transformation, int* host_ptr)
{
  ...
  cudaError err = cudaMemcpy(parameter_devptr, &parameter, sizeof(Parameter), cudaMemcpyHostToDevice);
  if (err != cudaSuccess)
    throw CudaException("DeviceLinear: unable to copy data from host to device.", err);
